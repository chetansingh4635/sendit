...
var route = new Route('post', '/secure/test');
...
//     logger.error("------------Debugging for startDiscussion Starts--------------------");
//     var condition = {
...
**/
route.use(function(req, res, next){
    //var indexOfAt = req.body.emailId.indexOf('@');
    //console.log(req.body.emailId.substring(0, indexOfAt));
    discourse.getUserEmail(req.body.emailId, function(error, success){
        console.log(error);
        console.log(success);
        if(!error) {
...
//             if(success && success.statusCode === config.discourseSuccessCode ){
//                 logger.error(success);
...
//                 res.json(errors.unable_to_register().withDetails(messages.discourse_register_message));
//             }
//         });
...
route.use(function(req, res, next){
    var indexOfAt = req.body.emailId.indexOf('@');
    discourse.getUser(req.body.emailId.substring(0, indexOfAt), function(error, success){
        console.log(error);
         console.log(success);
        if(!error) {
            logger.error(success);
